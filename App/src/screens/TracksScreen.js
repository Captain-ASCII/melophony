            ...
            <div id="contentHeader">
              ...
              <div id="toolBar">
                ...
                <div id="sortBar" >
                  ...
                  <Switch
                    enabledState={{ value: 'ASC', icon: 'sort-amount-up' }} disabledState={{ value: 'DESC', icon: 'sort-amount-down' }}
                    doubleState onSwitch={e => this.switchOrder(e)} configurationSwitch="sortOrder"
                  />
                </div>
                <div className="displayActions">
                  <Switch
                    icon="random" title="Switch track playing mode" active={configurationManager.get('shuffleMode')}
                    configurationSwitch="shuffleMode"
                  />
                  <i className="fa fa-list icon button" onClick={_ => this.changeTrackDisplay('itemList')} title="Track list"  />
                  <i className="fa fa-th icon button" onClick={_ => this.changeTrackDisplay('itemBlocks')} title="Track blocks"  />
                  <i className="fa fa-stream icon button" onClick={_ => this.changeTrackDisplay('groupedItems')} title="Tracks for each artist"  />
                </div>
              </div>
            </div>
            <div className="delimiter" />
            <TrackList
              tracks={this.state.tracks} filter={this.state.filter}
