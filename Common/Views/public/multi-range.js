...
class InputRange {
    ...
    constructor(name, element, track, multiRange = false) {
        ...
        if (multiRange) {
            content += `<input type='range' min="0" max="${track.duration}" value="${track.startTime}" step="0.5" oninput="modifyTrackEnd('${track.id}', this.value);" />`;
        }

        content += `
            <input class="mainRange" type='range' min="${multiRange ? 0 : track.startTime}" max="${multiRange ? track.duration : track.endTime}" value="${track.endTime}" step="0.5" oninput="modifyTrackStart('${track.id}', this.value);" />
